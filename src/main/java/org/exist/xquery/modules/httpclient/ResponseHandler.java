...
public class ResponseHandler {
    ...
    public static Sequence buildResult(final HttpResponse<byte[]> response,
        ...
        // Add body content (unless status-only or no body)
        if (!statusOnly && response.body() != null && response.body().length > 0) {
            byte[] bodyBytes = response.body();

            // Handle gzip decompression
            final String contentEncoding = response.headers()
                    .firstValue("Content-Encoding").orElse("");
            if ("gzip".equalsIgnoreCase(contentEncoding)) {
                try {
                    bodyBytes = new GZIPInputStream(
                            new ByteArrayInputStream(bodyBytes)).readAllBytes();
                } catch (IOException e) {
                    throw new XPathException(callerFunc,
                            HttpClientModule.HC002, "Error decompressing gzip response: " + e.getMessage());
                }
            }

            // Check for multipart
            if (mediaType.startsWith("multipart/")) {
                addMultipartBodies(result, bodyBytes, rawContentType, context, callerFunc);
            } else {
                addBody(result, bodyBytes, rawContentType, context, callerFunc);
            }
        }
    ...
    private static void addBody(final ValueSequence result, final byte[] bodyBytes,
                                 final String contentType, final XQueryContext context,
                                 final BasicFunction callerFunc) throws XPathException {
        if (ContentTypeHelper.isXml(contentType)) {
            result.add(parseXml(bodyBytes, contentType, context, callerFunc));
        } else if (ContentTypeHelper.isHtml(contentType)) {
            result.add(parseHtml(bodyBytes, contentType, context, callerFunc));
    ...
    private static Item parseXml(final byte[] bodyBytes, final String contentType,
                                   final XQueryContext context,
                                   final BasicFunction callerFunc) throws XPathException {
        try {
    ...
    private static Item parseHtml(final byte[] bodyBytes, final String contentType,
        ...
        try {
            return parseXml(bodyBytes, contentType, context, callerFunc);
    ...
    private static void addMultipartBodies(final ValueSequence result, final byte[] bodyBytes,
        ...
        for (final byte[] part : parts) {
            ...
            if (partBody.length > 0) {
                addBody(result, partBody, partContentType, context, callerFunc);
            }
        }
    }
